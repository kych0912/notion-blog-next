...
export const login = async (data: { id: string; password: string }) => {
  try {
    const res = await axios.post(
      `${process.env.NEXT_PUBLIC_BASE_API_URL}/api/user/login`,
      data
    );
    return res;
  } catch (err) {
...
export const getUserInfoAndPostByName = async (name: string) => {
  const res = await axios.get(
    `${process.env.NEXT_PUBLIC_BASE_API_URL}/api/user?name=${name}`
  );
  return res.data;
};

export const getUserInfoById = async (id: string | undefined) => {
  try {
    const res = await axios.get(
